function [RMSEtrain,RMSEtest,WAB,CB,SigmaB,WB,fB]=FCM_RDpA2(XTrain,yTrain,XTest,yTest,nFeatures,alpha,rr,P,gammaP,nRules,nIt,Nbs,WA0,C0,Sigma0,W0)
% %% Inputs:
...
for it=1:nIt
    ...
    for n=1:Nbs
        ...
        if ~sum(~isfinite(deltaYmu(:)))
            ...
            deltaW(idsKeep,:)=deltaW(idsKeep,:)+(yPred(n)-yTrain(idsTrain(n)))*fBar'*[1 XTrainB(n,:)];
            deltaXA(n,:)=deltaXA(n,:)+(yPred(n)-yTrain(idsTrain(n)))*fBar*W(idsKeep,2:1+nFeatures);
        end
    end
